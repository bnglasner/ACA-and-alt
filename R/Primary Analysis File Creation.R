...
using <- subset(NES, naics==industry_list[1])
...
using_model <- using_model %>% dplyr::select("estab_pop", 
                                             ...
                                             "Medicaid_lag2",
                                             "Marketplace.Type_na")
using_model_full <- using_model[complete.cases(using_model[,c(1:22)]),]
...
save(using, file = "medicaid_analysis_data_total.RData")
write.csv(using, file = "medicaid_analysis_data_total.csv") # Save this data as a .csv to load into stata for analysis
